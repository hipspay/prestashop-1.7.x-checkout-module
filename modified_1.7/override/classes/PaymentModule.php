...
abstract class PaymentModule extends PaymentModuleCore
{
    ...
    public function validateOrder(
            ...
            $secure_key = false, 
            Shop $shop = null, 
            $hipsShippingTaxIncl = null,
            $hipsShippingTaxExcl = null,
            $id_carrier_ps = null
    )
    {
        if (self::DEBUG_MODE) {
        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
            ...
            // Make sure CartRule caches are empty/var/www/LKPS173/classes/PaymentModule.php
            CartRule::cleanCache();
            ...
            foreach ($package_list as $id_address => $packageByAddress) {
                foreach ($packageByAddress as $id_package => $package) {
                    ...
                    if (isset($id_carrier_ps) && !empty($id_carrier_ps)) {
                        $id_carrier = $id_carrier_ps;
                        $order->id_carrier = $id_carrier_ps;
                        $carrier = new Carrier((int) $id_carrier_ps, (int) $this->context->cart->id_lang);
                    }
                    ...
                    if (isset($hipsShippingTaxExcl) && !empty($hipsShippingTaxExcl)) {
                        $order->total_shipping_tax_excl = (float) $hipsShippingTaxExcl;
                    } else {
                        $order->total_shipping_tax_excl = (float) $this->context->cart->getPackageShippingCost((int) $id_carrier, false, null, $order->product_list);
                    }
                    if (isset($hipsShippingTaxIncl) && !empty($hipsShippingTaxIncl)) {
                        $order->total_shipping_tax_incl = (float) $hipsShippingTaxIncl;
                    } else {
                    ...
                    if (isset($hipsShippingTaxExcl) && !empty($hipsShippingTaxExcl)) {
                        $order->total_paid_tax_excl = (float) $order->total_products + (float) $hipsShippingTaxExcl;
                    } else {
                    ...
                    if (isset($hipsShippingTaxIncl) && !empty($hipsShippingTaxIncl)) {
                        $order->total_paid_tax_incl = (float) $order->total_products_wt + (float) $hipsShippingTaxIncl;
                    } else {
                    ...
                    //echo 'CAR TOTAL PAID ' . number_format($cart_total_paid, _PS_PRICE_COMPUTE_PRECISION_) . '<br/>';
                    //echo 'AMOUNT PAID' . number_format($amount_paid, _PS_PRICE_COMPUTE_PRECISION_);
                    //die();
                ...
                if (isset($order->id)) {
                    ...
                    if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id) {
                        ...
                        $data = array(
                            ...
                            '{total_shipping}' => (isset($hipsShippingTaxIncl) ? Tools::displayPrice($hipsShippingTaxIncl, $this->context->currency, false) : Tools::displayPrice($order->total_shipping, $this->context->currency, false)),
